...
public class NineGridAdapterCZ extends CommonAdapter<String> {
    /**
     * 删除小弹窗
     */
    ...
    public NineGridAdapterCZ(Context context, List<String> selectPath, RecyclerView rvImages) {
        super(context, R.layout.shizhai_chuangzuo, selectPath);
    ...
    private void initTransfer(RecyclerView rvImages) {
        ...
        config = TransferConfig.build()
                ...
                .bindRecyclerView(rvImages, R.id.iv_sz_cz_photo);
    }
    ...
    protected void convert(ViewHolder viewHolder, String item, final int position) {
        ImageView ivThum = viewHolder.getView(R.id.iv_sz_cz_photo);
        ImageView ivAdd = viewHolder.getView(R.id.iv_sz_cz_add);
        if (item.equals("")) {
            //item为添加按钮
    ...
    private class PicturesClickListener implements View.OnClickListener {
        ...
        public void onClick(View view) {
            switch (view.getId()) {
                case R.id.iv_sz_cz_photo:
                    //点击图片
                    config.setNowThumbnailIndex(position);
                    config.setSourceImageList(getDatas());
                    transferee.apply(config).show();
                    break;
                case R.id.iv_sz_cz_add:
                    //点击添加按钮
                    if (listener != null)
                        listener.onAdd();
                    break;
